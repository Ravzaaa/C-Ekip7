...
ModeManager::ModeManager() : currentState(State::Normal) {}

// --- getters ---
State ModeManager::getCurrentState() const {
    return currentState;
}

// --- snapshot handling (LLR 4.2) ---
Snapshot ModeManager::saveSnapshot() const {
    return Snapshot(currentState);
}

void ModeManager::restoreSnapshot(const Snapshot& s) {
    currentState = s.getSavedState();
    applyRestrictions();
}

// --- state change (LLR 4.2 + LLR 4.5) ---
void ModeManager::setState(State s) {
...
// --- revert (LLR 4.3) ---
void ModeManager::revertToPreviousState() {
    if (history.empty()) {
...
void ModeManager::applyRestrictions() const {
